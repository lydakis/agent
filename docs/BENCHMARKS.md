...
512 MiB sampled RSS guard per tree, and 16-process guard. Claude Code runs one
native process per agent, so its guards are 512 MiB and 16 processes per agent
(see [Claude Code adapter](#claude-code-adapter)). Counters are sampled
