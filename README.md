...
**Status: experimental Rust execution core and durable JSONL service, with
Pi/Codex comparison tooling.** The prototype supports named bots, restart/resume,
historical checkpoints, cancellation, replay, and optional bounded shell execution.
Its provider interface and software protocol are experimental. Socket attachment,
automatic daemon startup, dedicated file-edit tools, and compaction are not implemented.
...
Next: separate long-term history storage from bounded model context, validate
live-provider admission, and add socket attachment. Keep matched
regression workloads as tools and durable lifecycle behavior expand.
